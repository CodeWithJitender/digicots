...
function HeroHeading({ setComponentLoaded }) {
  const headingRef = useRef(null);
  gsap.registerPlugin(ScrollTrigger);
  useGSAP(() => {
    setComponentLoaded((prev) => ({ ...prev, heroHeading: true }));
    ...
    >
      {
        <div className="container-xxl">
          <h1 className="sm:text-6xl overflow-hidden md:text-[6vw] text-[10vw] lg:text-[7vw] font-black text-white font-inter uppercase audiowide-regular">
            <TextAnimation1 animeStart="70" duration={2}>
              Things We Do
            </TextAnimation1>
          </h1>
          <p className="text-white raleway">
            <TextAnimation2 animeStart="70" duration={1.3}>
              We specialize in personalized and conversational marketing,
              crafting tailored experiences for every business.
            </TextAnimation2>
          </p>
        </div>
      }
