...
export default function CaseStudy({ slide, onClose }) {
  return (
    <div className="fixed inset-0 bg-opacity-60 bg-black flex h-[100vh] z-[1000] overflow-y-scroll">
      <div className="bg-[#EBEBEB] w-full grid grid-cols-[40%_60%] relative">
        {/* Left: Content */}
        <div className="h-dvh flex flex-col justify-between p-6 overflow-y-auto">
          <div>
            <div className="flex justify-between mb-10">
              <img src="logo-black.png" className="max-w-[200px]" alt="Logo" />
              <button onClick={onClose} className="text-xl text-gray-600 hover:text-black cursor-pointer">
                <img src="cross.png" className="max-w-[40px]" alt="Close" />
              </button>
            </div>

            <h2 className="text-5xl font-bold text-[#242424] mb-6">{slide.title}</h2>
            ...
            {slide.section?.map((group, idx) => (
              <div key={idx} className="mb-10">
                {group.map((item, i) => {
                  if (item.key === "title") {
                    return <h3 key={i} className="text-2xl font-bold mb-3">{item.value}</h3>;
                  }
                  if (item.key === "subH") {
                    return <h4 key={i} className="text-lg font-semibold mb-2 text-orange-500">{item.value}</h4>;
                  }
                  if (item.key === "p") {
                    return <p key={i} className="text-gray-700 mb-3">{item.value}</p>;
                  }
                  if (item.key === "ul") {
                    return (
            ...
            <div className="flex gap-3 flex-wrap mt-6">
              {slide.tags.map((text, index) => (
                <span key={index} className="bg-[#ED510C] text-white px-4 py-2 rounded-[20px] font-inter font-bold text-sm">
                  {text}
                </span>
              ))}
            </div>
